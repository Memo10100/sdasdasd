...
if (isset($_POST['comprar'])) {
    ...
    if (mysqli_num_rows($carrito_query) > 0) {
        while ($carrito_item = mysqli_fetch_assoc($carrito_query)) {
            $carrito_products[] = $carrito_item['pnombre'] . ' (' . $carrito_item['pcantidad'] . ') (' . $carrito_item['pprecio'] . ') ';
...
<body>
    ...
    <section class="display-order">
        ...
        if (mysqli_num_rows($select_carrito) > 0) {
            while ($fetch_carrito = mysqli_fetch_assoc($select_carrito)) {
                ...
                ?>
                <p>
                    <?php echo $fetch_carrito['pnombre'] ?> <span>(
                        <?php echo '$' . $fetch_carrito['pprecio'] . '/-' . ' x ' . $fetch_carrito['pcantidad'] ?>)
                    </span>
                </p>
                <?php
            }
        ...
        ?>
        <div class="grand-total">Total final : <span>$
    ...
    <section class="checkout">
        ...
        <form action="" method="POST">
            ...
            <input type="submit" name="comprar" value="Confirmar Compra" class="btn">
